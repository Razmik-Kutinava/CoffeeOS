...
**Фаза:** PHASE 0 `[x]` · SPEC `[x]` · шаги 1–2 GREEN `[x]` · RED шаг 3 `in_progress` · REVIEW `[ ]`  
**CBR:** #40
...
| 3 | `accepted` + succeeded → Cancel → `refunded` + `cancelled` | guest cancel unit + stub adapter; integration `orders_guest_cancel_test` | `GuestOrderCancellationService` (+ optional thin refund helper) | **RED** `[ ]` GREEN |
