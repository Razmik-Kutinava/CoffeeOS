...
|---|-----|------------------|-------|--------|
| 1 | Обогащение FCM payload: `tag`, unicode progress, `actions` по матрице; soft-fail | `order_status_push_notifier.rb` · payload builder (новый тонкий helper при >~50 строк) · `fcm_client.rb` (android/web fields если нужны) · `ready_push_job.rb` (тот же контракт) | `test/services/shop/order_status_push_notifier_test.rb` (+ helper unit) | `[ ]` |
