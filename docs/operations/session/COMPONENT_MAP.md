...
| PaymentResult | app/frontend/routes/PaymentResult.svelte | Маршрут #/payment-result: success/fail/SBP waiting; email-сбор #71; переход на `#/` | → OrderSuccessEmailBlock; emailCollection; shopSbpPay; api profile; push `/` | #93, #71, #35 | `maybeAutoReturnToCatalog` только при `!askReceiptEmail` (#93); serverEmail из `api("profile")` + LS fallback (#71 Патч_1); Continue → `handleEmailSkip` (#35); SBP waiting/recover не ломать (#79/#86) |
