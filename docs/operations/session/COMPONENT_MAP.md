...
| ActiveOrdersAccordion | app/frontend/components/ActiveOrdersAccordion.svelte | Строка заказа: meta + progress + CTA + X (крестик); клик → #/order/:id | → OrderActionButtons; lib activeOrdersAccordion (receiptView — не рендерится сейчас) | #81, #35, #77, #63 | Дочерний компонент OrderStatusSheet; крестик здесь; чек не подключён, хотя хелпер есть |
...
| orderStatusSheet.js | app/frontend/lib/orderStatusSheet.js | Стейт peek/hidden/visible/dismiss, poll, cable apply | OrderStatusSheet, CartSheet | #82, #63 | — |
