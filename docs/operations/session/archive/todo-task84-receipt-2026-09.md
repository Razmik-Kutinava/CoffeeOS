# todo — TASK_84-RECEIPT-DISPLAY-EXT (архив ctx-trim 2026-09-22)
...
2. `app/frontend/components/ActiveOrdersAccordion.svelte` — фактический рендер/путь раскрытия receipt
3. `app/frontend/lib/activeOrdersAccordion.js` — **только если** сломан expand/`activeExpandedOrderId`; **не** переписывать `receiptView`
4. `app/frontend/lib/orderStatusNotifyActions.js` — **сосед:** только если сломан `openOrderReceipt`
