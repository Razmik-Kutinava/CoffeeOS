...
### Bottom sheet expanded grid (2026-07-21) — RESTART

| Что | Статус |
|-----|--------|
| ТЗ + скрин заказчика (intake `948648e`) — текст 1:1 сохранён | **`[x]`** |
| RED `273a43c` → GREEN `7683dee` (grid в шторке) | **откачено** — владелец уточнил: сетка = **каталог**, не шторка |
| Откат кода к до-RED (prog26) · регрессия cart sheet 56/0 | **`[x]`** |
| Уточнённое ТЗ / решение по каталогу (4-в-ряд на 414px ≈ 93px карточки) | **`[ ]`** — ждём владельца |
| Новое SPEC → RED → GREEN | **`[ ]`** |
