...
**Чеклист закрытия В1:** `docs/operations/milestones/veha_1/checklists/CHECKLIST.md` — основной рабочий документ; отмечать `[x]` по мере готовности.

### Gate: чеклист ↔ таск-трекер (не повторять «In Progress» при готовом коде)

1. **Источник решений** по смене/заказам: `docs/operations/milestones/veha_1/reference/ORDER_ENTRY_AUDIT.md` (реестр входов + A/B).
...
Блок **G** — **закрыт**. **H.2** — **закрыт** (2026-05-25). **Git + Fly** — push develop, fix npm v1.53 (`4a25187`). **Веха 1 ЗАКРЫТА** 2026-06-19 — заочная приёмка по апруву владельца (H.3 §1, § I `[x]`). **Веха 2** — активная разработка (`docs/operations/session/HANDOFF.md`).
...
**Закрыто 2026-06-19:** § H.3 (заочно, апрув владельца); § I ops. Хвосты → В2 (см. CHECKLIST § I).

### H.3 — передача заказчику (стенд готов) — **2026-05-26**

- Fly smoke: `/up`, витрина A/B (`tenant_id`), логин barista — OK.
- Одностраничник: `docs/operations/demo/CUSTOMER_HANDOFF.md`.
...
**Логины:** `docs/operations/milestones/veha_1/reference/DEMO_LOGINS.md`, пароль `demo123456`.  
...
**Demo-пользователи** (пароль `demo123456`): полная таблица — **`docs/operations/milestones/veha_1/reference/DEMO_LOGINS.md`**. Кратко:
