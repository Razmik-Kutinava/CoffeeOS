...
|---|-------|----------------------------|--------|
| 2A.1 | **Health JSON** | API `/health/tenants` — JSON по каждой точке: смена, заказы, очередь, оплаты, отказы. Проверить на Fly, показать УК, допилить поля если не хватает | `[ ]` |
