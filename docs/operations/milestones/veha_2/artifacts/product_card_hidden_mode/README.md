# Артефакты: режим Hidden для карточек товаров

**Что это за папка:** все скрины, MCP-прогоны и файлы приёмки **только** для задачи  
«Исправление режима отображения Hidden для карточек товаров».

| | |
|--|--|
| **ТЗ (канон текста заказчика)** | [`../../requirements/customer_tasks/Исправление режима отображения Hidden для карточек товаров.md`](../../requirements/customer_tasks/Исправление%20режима%20отображения%20Hidden%20для%20карточек%20товаров.md) |
| **Скрины заказчика** | [`screenshots/`](screenshots/README.md) |
| **Код** | не трогаем до апрува ТЗ / **go** |

### Зачем папка

Чтобы не смешивать эталон «как нужно» и баг «как сейчас» с другими задачами витрины (`product_card_peek_cart`, `demo-feedback`, UserCards и т.д.).

Канон хранения скринов этой задачи: **эта папка** → `screenshots/`.
