...
| **Статус** | **REVIEW** · Next: CI / deploy апрув |
| **Ветка** | `develop` |
...
| **R3 (K3)** | `adapter_payment_url`: production — **не** fake tinkoff URL. Blank → `""` (charge/one-click). Non-http non-blank → raise 422. Dev/test — fallback ок. `SHOP_BASE_DOMAIN` blank → `?tenant_id=` |
...
- [x] PHASE 3 `/review` — таблица K1–K7 · K6 sha sync · push · **без deploy**
