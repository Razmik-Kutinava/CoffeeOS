...
|------|----------|------------------|------|--------|-------------|
| 2026-09-07 | заказчик правки п.10 | ready → SMS + гл. экран | Каскад SMS после «заказ готов» не работает; статусная модель не пропадает с гл. экрана после «готов» на табло | **SPEC** · CBR #82 · `/sbr` pending | [ТЗ](customer_tasks/Каскад%20SMS%20после%20Заказ%20готов%20и%20шторка%20статуса%20на%20главном.md) |
| 2026-09-26 | ops / Патч 1 Задача-1 | Point A CTA `ready` | **Патч v2:** экстренное отключение только через `enabled=false`; `second_cta_mode` не механизм disable (Point A: tips→subscription, enabled остаётся false). Других risky точек нет. Код CTA/eligibility / `INTEGRATIONS.md` не трогали. | **done** *(config Fly + CTA matrix test)* | [`rollback_point_a_patch_v2_2026-09-26.json`](../artifacts/subscription_offer_eligibility/ops/rollback_point_a_patch_v2_2026-09-26.json) |
| 2026-09-05 | ops / владелец | Point A CTA `ready` | **Временный откат** subscription-оффера: billing/экран ещё нет. УК → `enabled=false` + `second_cta_mode=tips`. Visible risky points: только Point A. Код CTA/eligibility не трогали. `INTEGRATIONS.md` без изменений. | **done** *(superseded Патч 1 2026-09-26: tips больше не механизм)* | [`rollback_point_a_2026-09-05.json`](../artifacts/subscription_offer_eligibility/ops/rollback_point_a_2026-09-05.json) |
...
| 2026-07-21 | MCP Fly Quick Repeat (real) | полный E2E без стабов | Реальный прогон: посеян клиент с историей, вход email-OTP, секция повтора из реального API, счётчики/«повторить»/«оплатить в 1 клик» — 8/8 PASS + чеклист заказчику | **ждёт апрув заказчика** | [fly_real_run_mcp_2026-07-21.json](artifacts/quick_repeat_bottom_sheet/fly_real_run_mcp_2026-07-21.json) |
| 2026-07-21 | MCP Fly Quick Repeat (real) | checkout (Оформление) | **UX-3:** шторка корзины с секцией повтора перекрывает форму email на Оформлении (скрин 05 real_run); предложение — скрывать секцию повтора на checkout. Плюс NOTE: «оплатить в 1 клик» упирается в guard «Укажите email», пока клиент не подтвердил email в форме | open | [fly_real_run_mcp_2026-07-21.json](artifacts/quick_repeat_bottom_sheet/fly_real_run_mcp_2026-07-21.json) |
