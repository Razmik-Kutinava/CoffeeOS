...
**Стадия (2026-06-06):** **§2.3 закрыт**, **блок 2A закрыт**. **Следующий фокус:** **блок 2** (табло баристы).
...
| № | Поток | Суть | Сейчас |
|---|-------|------|--------|
| **1** | **Приём заказа** | Витрина (mobile first → PWA → app → kiosk): меню, модификаторы, цены, оплата, регистрация, уведомления, юр. | **§2.3 done**; хвосты меню/цен/модификаторов — **волна 1** |
| **2** | **Обработка заказа** | Табло баристы (Flutter или web): заказы, статусы, отмена/возврат, стоп-лист. **Плюс УК:** журнал, логин, транзакции, мониторинг | **2A done** (УК); **табло — `[ ]`** |
| **3** | **Тест-приёмка** | Сквозной happy path: витрина → табло → уведомления; изоляция точек; возвраты; **как выглядит отказ** на каждом экране | **`[ ]`** — после потоков 1–2, по PDF §1–§8 |
...
| **4** | **сейчас** | **Поток 1 — хвосты витрины** (волна 1 ниже) → **поток 2 табло** | После закрытия §2.3 и 2A возвращаемся к **меню/ценам** из PDF | **Возврат** к §2.1–2.2, §1.3 и сценариям 56 стр. |
| **5** | далее | **Поток 3** — сквозной happy path + отказы везде | Приёмка по PDF целиком | **Главный финиш** — весь PDF как эталон |
...
|---|--------|------------|--------|------------|
| W1.1 | **Баг 6.1** — цена из УК/менеджера **не доходит** на витрину | **нет** (баг open) | `[ ]` | Вероятно: нет cache bust при `manager/menu#update_price`; УК сбрасывает через `ProductTenantSync.bust_shop_catalog_cache!` |
| W1.2 | **MCP:** из УК категория + 2 товара + фото + модификаторы → **проверка на витрине** | частично | `[ ]` | Автообновление ~8 s — **done**; полный сценарий «создал с нуля» — **нет** |
| W1.3 | **Обязательные модификаторы** — на тестовом контенте | не проверено | `[ ]` | Код может быть; нужен MCP на Fly |
| W1.4 | **Сверка категорий:** ядро = витрина = barista | частично (~90%) | `[ ]` | `Shop::Catalog.products_scope` + PTS; расхождения — open |
...
|-------|--------|---------|
| **1 Приём** | Оплата §2.3, autorefresh меню, онбординг базово | W1.1–W1.4, уведомления, юр., kiosk |
...
|---|-------------|------------|--------|
| **1** | Приём заказа | Витрина (mobile first) → app → kiosk | **§2.3 done**; W1.1–W1.4 open |
...
| Поле | Значение |
|------|----------|
| Версия | 2026-06-06, **§2.3 + 2A закрыты**; **волна 4** — поток 1 (W1.1–W1.4) |
| **Следующий шаг** | **W1.1** баг цены → W1.2 MCP меню → **блок 2** табло |
