...
| 93 | **Post-pay auto return → catalog + status (TASK_91)** | п.9.2: после email/Skip на payment-result → авто `#/` + существующая статусная модель; защита #71 no immediate redirect | **SPEC `[x]`** 2026-09-17 · ждёт `/sbr` | [TASK-91-…](customer_tasks/TASK-91-Автоматический-возврат-на-каталог-после-post-pay-email.md) · [artifacts](../artifacts/post_pay_auto_return_catalog_status/) |
| 94 | **Production background FCM + status sync (TASK_92)** | п.1 фоновые FCM не реализованы; чат поддержки UI есть, клик нет; reopen #38/#81; CBR #92 ≠ этот TASK | **intake** 2026-09-17 · ждёт `go` → `/spec` | [TASK-92-…](customer_tasks/TASK-92-Production-background-FCM-и-синхронизация-статусов-заказа.md) · [artifacts](../artifacts/production_background_fcm_status_sync/) |
