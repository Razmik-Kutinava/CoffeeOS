...
| PWA status sync + repeats stale | **Витрина / PWA** — статусы с табло баристы не едут без reload; после заказа «повторы»/история не подгружаются (пустая шторка +0₽) | [ТЗ](customer_tasks/Статусы%20с%20табло%20не%20подтягиваются%20в%20PWA%20и%20повторы%20после%20заказа.md) · [`artifacts/pwa_status_sync_and_repeats_stale/`](../artifacts/pwa_status_sync_and_repeats_stale/) | **GREEN+REVIEW `[x]`** 2026-08-08 · MCP/апрув `[ ]` |
...
| 47 | **PWA status sync + repeats stale** | Статусы с табло не в PWA без reload; после заказа повторы/история не подгружаются | **GREEN+REVIEW `[x]`** · MCP `[ ]` | [Статусы с табло не подтягиваются…](customer_tasks/Статусы%20с%20табло%20не%20подтягиваются%20в%20PWA%20и%20повторы%20после%20заказа.md) · [artifacts](../artifacts/pwa_status_sync_and_repeats_stale/) |
