...
| Repeat order invalid token payment sheet | **Витрина** — повторный заказ / invalid token + **G7** insufficient → NewCardForm + UI скрин 03 (G1–G4) | [ТЗ](customer_tasks/Главный%20экран%20—%20повторный%20заказ%20(невалидный%20токен)%20BottomSheet%20выбора%20способа%20оплаты.md) · [`artifacts/…`](../artifacts/repeat_order_invalid_token_payment_sheet/) · скрины `03`–`08` | **local GREEN `[x]`** 2026-08-07 · MCP Fly `[ ]` · апрув заказчика `[ ]` |
...
| 26 | **Repeat order invalid token payment sheet** | G7 NewCardForm + UI скрин 03 | **local GREEN `[x]`** 2026-08-07 · MCP `[ ]` · апрув `[ ]` | [Главный экран — повторный заказ…](customer_tasks/Главный%20экран%20—%20повторный%20заказ%20(невалидный%20токен)%20BottomSheet%20выбора%20способа%20оплаты.md) |
