...
| Repeat order invalid token payment sheet | **Витрина** — повторный заказ при невалидном токене: CTA «Добавить карту», BottomSheet «Способ оплаты», inline-ошибки · **live 2026-08-07:** insufficient funds → форма новой карты не открывается (G7) | [ТЗ](customer_tasks/Главный%20экран%20—%20повторный%20заказ%20(невалидный%20токен)%20BottomSheet%20выбора%20способа%20оплаты.md) · [`artifacts/…`](../artifacts/repeat_order_invalid_token_payment_sheet/) · скрины `03`–`08` | **MCP PASS `[x]`** v393 · **live gap G7 `[ ]`** · апрув заказчика `[ ]` |
...
| 26 | **Repeat order invalid token payment sheet** | Peek повтор + sheet; **live:** decline insufficient funds → открыть NewCardForm (G7) | **MCP v393 `[x]`** · **G7 `[ ]`** · скрины 03–08 2026-08-07 · апрув `[ ]` | [Главный экран — повторный заказ…](customer_tasks/Главный%20экран%20—%20повторный%20заказ%20(невалидный%20токен)%20BottomSheet%20выбора%20способа%20оплаты.md) |
