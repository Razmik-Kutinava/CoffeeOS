...
| **Этот файл** | **Новые/уточнённые** требования + **чеклисты**. Сейчас: **блок 2** (табло) · backlog потока 1 — ниже. |
...
| # | Что | Зачем | Приоритет |
|---|-----|-------|-----------|
| B1.1 | **Уведомления гостю** — экран статуса + прогресс-бар | Поток 3; PDF §1.7; **требование заказчика ниже** | **высокий** |
| B1.2 | **Юр.** — оферта, согласия на витрине | Регистрация / оплата | средний |
| B1.3 | **Чек / ОФД на почту** | После оплаты | средний |
| B1.4 | **PWA / app / kiosk** | Отдельные каналы; kiosk ≠ barista sync | низкий |
...
| B1.7 | **Email verify** — checkout, OTP через **Brevo** | Код 2026-06-09; `BREVO_API_KEY` + `MAIL_FROM` | **`[x]` внутр. приёмка** · апрув заказчика `[ ]` |
...
## Следующее требование заказчика — B1.1 (2026-06, как прислали)

**Статус:** `[ ]` open · **приоритет высокий** · связь с **B1.1** · push — отдельно (вне scope ниже).
...
10. Без ввода кода подтвержения заказ не оформляется
...
**Доказательства:** [`checkout_acceptance_2026-06-09.json`](../artifacts/demo-feedback/checkout_acceptance_2026-06-09.json) · MCP-скрины 4 точек: [`screenshots/checkout_mcp_2026-06-09/`](../artifacts/demo-feedback/screenshots/checkout_mcp_2026-06-09/) · Fly: `ruby bin/shop_checkout_otp_fly_multitenant.rb`
...
|-------|--------|---------|
| **1 Приём** | Оплата §2.3, W1.1–W1.5, онбординг | **backlog B1.1–B1.7** (уведомления, юр., kiosk…) |
...
| 1.7–1.8 | Уведомления, онбординг | уведомления open → **B1.1**; онбординг done |
