...
| Order ready cascade WS→TG→SMS | **Витрина / уведомления** — каскад «Заказ готов»: WS presence → Telegram Bot → SMS.ru (≤70); секреты из `.env`; Sidekiq fault-tolerance | [ТЗ](customer_tasks/Оптимизированный%20каскад%20уведомлений%20Заказ%20готов%20PWA%20WS%20Push%20Telegram%20SMS.md) · [`artifacts/order_ready_cascade_ws_telegram_sms/`](../artifacts/order_ready_cascade_ws_telegram_sms/) | **интейк `[x]`** 2026-08-03 · SPEC `[ ]` ждёт go |
...
| 39 | **Order ready cascade WS→TG→SMS** | Каскад «Заказ готов»: WS/Push/Wallet → presence → Telegram Bot → SMS.ru (≤70); Sidekiq; секреты `.env` | **интейк `[x]`** 2026-08-03 · SPEC `[ ]` ждёт go | [Оптимизированный каскад уведомлений…](customer_tasks/Оптимизированный%20каскад%20уведомлений%20Заказ%20готов%20PWA%20WS%20Push%20Telegram%20SMS.md) |
