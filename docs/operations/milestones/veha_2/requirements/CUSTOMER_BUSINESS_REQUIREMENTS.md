...
| 26 | **Repeat order invalid token payment sheet** | Peek повтор: CTA «Добавить карту» + BottomSheet способов оплаты при invalid token | **интейк `[x]`** 2026-07-27 · SPEC `[ ]` ждёт go | [Главный экран — повторный заказ…](customer_tasks/Главный%20экран%20—%20повторный%20заказ%20(невалидный%20токен)%20BottomSheet%20выбора%20способа%20оплаты.md) |
