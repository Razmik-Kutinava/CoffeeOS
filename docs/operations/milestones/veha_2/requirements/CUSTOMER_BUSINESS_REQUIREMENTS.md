...
| 14 | **Quick Repeat Bottom Sheet** | Быстрый повтор частых покупок — секция «повторить», peek/expanded/hidden, «в 1 клик»; скрывать при активном заказе; status sheet full-width | **REVIEW `[x]`** 2026-07-31 · MCP/deploy `[ ]` | [Быстрый повтор частых покупок Quick Repeat Bottom Sheet.md](customer_tasks/Быстрый%20повтор%20частых%20покупок%20Quick%20Repeat%20Bottom%20Sheet.md) |
