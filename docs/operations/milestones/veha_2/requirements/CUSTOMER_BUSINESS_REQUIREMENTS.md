...
| Email after pay Callcheck (#71) | **Витрина / PWA** — Патч_1: post-pay email → `MobileCustomer.email` + server prefill (не только LS); не спрашивать на следующих | [ТЗ](customer_tasks/Email-сбор%20после%20оплаты%20(Callcheck-флоу).md) § Патч_1 · [`artifacts/email_collection_after_payment/`](../artifacts/email_collection_after_payment/) | **Патч_1** `/patch` ready · Next `/sbr` |
...
| 89 | **PWA auth registration Callcheck → SMS** | Callcheck→SMS; linker/session; post-call → PWA checkout + экран оплаты; overlap #80 UI | **REVIEW** 2026-09-16 · push/CI · G5 Fly после deploy | [TASK-89-…](customer_tasks/TASK-89-Авторизация-регистрация-PWA-Callcheck-SMS.md) · [artifacts](../artifacts/pwa_auth_registration_callcheck_sms/) |
| 90 | **Callcheck return → continue payment (POSTCALL-EXT)** | EXT #89: return из телефона → resume Callcheck → auth → PaymentMethodsSheet; без UI-EXT / backend | **REVIEW** 2026-09-16 · push/CI · G5 Fly после deploy | [TASK-89-POSTCALL-EXT-…](customer_tasks/TASK-89-POSTCALL-EXT-Возврат%20в%20PWA%20после%20Callcheck%20и%20продолжение%20оплаты.md) · [artifacts](../artifacts/pwa_callcheck_return_continue_payment/) |
| 91 | **PWA auth phone input UI/UX (UI-EXT)** | EXT #89: phone-auth — скрыть CTA суммы + cart preview; тоньше sheet; без Callcheck/SMS | **REVIEW** 2026-09-16 · push/CI · G5 Fly после deploy | [TASK-89-UI-EXT-…](customer_tasks/TASK-89-UI-EXT-UI-UX-авторизации-PWA-экран-ввода-телефона.md) · [artifacts](../artifacts/pwa_auth_phone_input_ui_ux/) |
| 92 | **WebPush recovery after denied (TASK_90)** | EXT #81 п.9.1: recovery UI + deep-link/fallback `denied → settings`; без backend/SW/iOS | **REVIEW** 2026-09-17 · push/CI · G5 Fly после deploy | [TASK-90-…](customer_tasks/TASK-90-Восстановление-WebPush-после-запрета-уведомлений.md) · [artifacts](../artifacts/webpush_recovery_after_denied/) |
| 93 | **Post-pay auto return → catalog + status (TASK_91)** | п.9.2: после email/Skip на payment-result → авто `#/` + существующая статусная модель; защита #71 no immediate redirect | **REVIEW** 2026-09-17 · push/CI · G4 Fly после deploy | [TASK-91-…](customer_tasks/TASK-91-Автоматический-возврат-на-каталог-после-post-pay-email.md) · [artifacts](../artifacts/post_pay_auto_return_catalog_status/) |
| 94 | **Production background FCM + status sync (TASK_92)** | п.1 фоновые FCM не реализованы; чат поддержки UI есть, клик нет; reopen #38/#81; CBR #92 ≠ этот TASK | **REVIEW** 2026-09-17 · push/CI · G5 Fly после deploy | [TASK-92-…](customer_tasks/TASK-92-Production-background-FCM-и-синхронизация-статусов-заказа.md) · [artifacts](../artifacts/production_background_fcm_status_sync/) |
