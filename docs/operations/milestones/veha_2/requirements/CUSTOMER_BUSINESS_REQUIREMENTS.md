...
| Order status compact sheet + Push | **Витрина / PWA + POS** — сквозная sticky-шторка статуса (peek/hidden) + ActionCable + Push/Wallet из табло; non-blocking каталог | [ТЗ](customer_tasks/Интеграция%20статусной%20модели%20в%20компактную%20шторку%20PWA%20и%20Push.md) · [`artifacts/order_status_compact_sheet_push/`](../artifacts/order_status_compact_sheet_push/) · [Wallet runbook](../runbooks/APPLE_WALLET_ORDER_PASS.md) | **REVIEW `[x]`** 2026-07-31 · MCP/deploy `[ ]` · PKCS7 backlog |
...
| Background FCM progress + Apple Wallet | **Витрина / PWA** — обогащённый FCM (tag/actions/прогресс) + SW actions; `.pkpass` + APNs update; UI state machine карточки (макс. 2 CTA) | [ТЗ](customer_tasks/Фоновые%20уведомления%20прогресс-бар%20Android%20FCM%20и%20Apple%20Wallet%20iOS.md) · [`artifacts/background_notifications_fcm_apple_wallet/`](../artifacts/background_notifications_fcm_apple_wallet/) | **push `[x]`** · Fly release **BLOCKED billing** · MCP `[ ]` |
| Order ready cascade WS→TG→SMS | **Витрина / уведомления** — каскад «Заказ готов»: WS presence → Telegram Bot → SMS.ru (≤70); секреты из `.env`; Solid Queue fault-tolerance | [ТЗ](customer_tasks/Оптимизированный%20каскад%20уведомлений%20Заказ%20готов%20PWA%20WS%20Push%20Telegram%20SMS.md) · [`artifacts/order_ready_cascade_ws_telegram_sms/`](../artifacts/order_ready_cascade_ws_telegram_sms/) | **SPEC `[x]`** 2026-08-03 · RED `[ ]` ждёт go |
...
| 35 | **Order status compact sheet + Push** | Сквозная sticky-шторка статуса (peek/hidden) + ActionCable из POS + Push/Wallet; non-blocking каталог | **REVIEW `[x]`** 2026-07-31 · MCP/deploy `[ ]` | [Интеграция статусной модели…](customer_tasks/Интеграция%20статусной%20модели%20в%20компактную%20шторку%20PWA%20и%20Push.md) |
...
| 38 | **Background FCM progress + Apple Wallet** | FCM tag/actions/прогресс + SW; `.pkpass` + APNs update; PWA UI state machine (макс. 2 CTA); без правок Barista status | **push `[x]`** · deploy **BLOCKED billing** · MCP `[ ]` | [Фоновые уведомления прогресс-бар…](customer_tasks/Фоновые%20уведомления%20прогресс-бар%20Android%20FCM%20и%20Apple%20Wallet%20iOS.md) |
| 39 | **Order ready cascade WS→TG→SMS** | Каскад «Заказ готов»: WS/Push/Wallet → presence → Telegram Bot → SMS.ru (≤70); Solid Queue; секреты `.env` | **SPEC `[x]`** 2026-08-03 · RED `[ ]` ждёт go | [Оптимизированный каскад уведомлений…](customer_tasks/Оптимизированный%20каскад%20уведомлений%20Заказ%20готов%20PWA%20WS%20Push%20Telegram%20SMS.md) |
