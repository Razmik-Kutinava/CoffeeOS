...
| SMS.ru API sms/send | **Интеграции / SMS.ru** — дока `sms/send`: параметры, json-ответ (`sms_id`), коды; enrich bridge; код клиента — после go | [ТЗ](customer_tasks/SMS.ru%20API%20Отправить%20СМС%20HTTP%20запросом.md) · [`artifacts/sms_ru_api_send_http/`](../artifacts/sms_ru_api_send_http/) | **интейк + bridge `[x]`** 2026-08-11 · SPEC/код `[ ]` |
...
| 48 | **SMS.ru API sms/send** | Дока HTTP `sms/send` → bridge; клиент: `sms_id` / per-phone ошибки (после go) | **интейк + bridge `[x]`** 2026-08-11 · SPEC/код `[ ]` | [SMS.ru API Отправить СМС HTTP…](customer_tasks/SMS.ru%20API%20Отправить%20СМС%20HTTP%20запросом.md) · [artifacts](../artifacts/sms_ru_api_send_http/) |
