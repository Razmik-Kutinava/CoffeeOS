# Каскад SMS после «Заказ готов» и шторка статуса на главном

**CBR:** #82 · **Дата intake:** 2026-09-07  
**Источник:** текст заказчика (правки п.10 + Google Doc) + баг шторки  
**Артефакты:** docs/operations/milestones/veha_2/artifacts/order_ready_cascade_sms_status_sheet_fix/  
**Связь:** канон каскада — [#39](Каскад%20уведомлений%20Заказ%20готов%20PWA%20WS%20Push%20WebPush%20Apple%20Wallet%20SMS.md) · hide on ready — [#35](Интеграция%20статусной%20модели%20в%20компактную%20шторку%20PWA%20и%20Push.md) · реактивность — [#63](Исправление%20реактивности%20Svelte%205%20и%20UX%20управления%20статусным%20виджетом.md)  
**Google Doc:** https://docs.google.com/document/d/134SH9AGyliv3IxhXJiXz0jZxZo6HI27nJiOeN-zaNdU/edit?usp=sharing
...
10/ каскад  сообщение смс после  стстуса "заказ готов" не рабортает

https://docs.google.com/document/d/134SH9AGyliv3IxhXJiXz0jZxZo6HI27nJiOeN-zaNdU/edit?usp=sharing
...
​Если валидация пройдена: Выполняется POST [https://sms.ru/sms/send](https://sms.ru/sms/send) с параметрами api_id, to, msg, json=1.
...
*** не пропадает с гл экрана статусная модель даже когда был отжат на табло баристы кнопка - готов заказ

---

## Заметки агента

- Полный Google Doc = канон **#39** (уже в репо с 2026-08-04; Fly v427 MCP SMS PASS). Это **reopen / регрессия**: «SMS после ready не работает».
- Второй пункт — регрессия **#35** / **#63**: виджет статуса должен скрываться на `ready`; заказчик: на гл. экране остаётся после кнопки «готов» на табло.
- Стек тестов в ТЗ (RSpec/WebMock) **не канон** CoffeeOS — при SPEC сверять с Minitest / существующими тестами каскада.
- Активный `todo.md` сейчас на **#79** (SBP return) — после `/spec` по #81 решить приоритет / переключение todo.
- Код на этом шаге не трогали.
