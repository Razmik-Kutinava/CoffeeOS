...
| **Bottom sheet expanded grid** | Expanded до 1-го ряда каталога + **сетка товаров 4 в ряд** со скроллом | **SBR REVIEW `[x]`** · код `[x]` · апрув / MCP / deploy `[ ]` | [Bottom Sheet expanded mode и внутренняя сетка 4 в ряд.md](Bottom%20Sheet%20expanded%20mode%20и%20внутренняя%20сетка%204%20в%20ряд.md) |
