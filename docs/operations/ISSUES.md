...
|------|-----------|--------|---------------|
| 2026-08-13 | **Pay NET_ERROR** сырой `Failed to fetch` в PaymentMethodsSheet | 🔴 open | Убрать alert/`sheetInlineError` = e.message при NET; оставить только «Нет связи. Повторить»; → полная запись |
| 2026-08-13 | **A6 Repeat** после cancel: `frequent_products.has_active_order=true` при `orders/active=[]` | 🔴 open | Cache/invalidate frequent после cancel; MCP v450 FAIL A6; → полная запись |
