...
# B1.12-R2 — prep Fly: verified email + card order + iframe session → JSON для MCP.
#
#   FLY_BIN=flyctl ruby bin/b112_r2_native_card_prep_fly.rb
#   node bin/b112_r2_native_card_mcp.mjs
#
# → tmp/b112_r2_native_card_prep.json
...
OUT = ENV.fetch("OUT", File.join(ROOT, "tmp/b112_r2_native_card_prep.json"))
...
email = "b112-r2-#{suffix}@coffeeos.dev"
name = "B112 R2 QA"
jar = File.join(Dir.tmpdir, "b112-r2-#{suffix}.cookies")
...
raise "no payment_url" unless order_body["payment_url"].to_s.length.positive?

config_body, config_http = curl_get_json("#{BASE}/shop/api/config?tenant_id=#{TENANT_ID}", jar, headers: headers)
raise "config failed #{config_http}" unless config_http == 200

prep = {
  scenario: "b112_r2_native_card",
  date: DATE,
  base: BASE,
  tenant_id: TENANT_ID,
