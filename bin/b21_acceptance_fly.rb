...
#   FLY_BIN=flyctl ruby bin/b21_acceptance_fly.rb
...
Dir.chdir(ROOT) do
  run!("FLY_BIN=#{FLY_BIN} ruby bin/b21_acceptance_prep.rb", "acceptance prep")
  prep = JSON.parse(File.read("tmp/b21_acceptance_prep.json"))

  playwright_ok = system("node bin/b21_acceptance_fly.mjs")
  ...
  smoke_ok = system("FLY_BIN=#{FLY_BIN} ruby bin/b21_barista_board_fly_smoke.rb")
  ...
  if ENV["B21_RUN_E2E"] == "1"
    e2e_ok = system("FLY_BIN=#{FLY_BIN} ruby bin/b21_mcp_e2e_fly.rb")
    e2e_path = Dir.glob("docs/operations/milestones/veha_2/artifacts/demo-feedback/b21_mcp_e2e_*.json").max
  ...
  result = {
    ...
    scripts: {
      b21_acceptance_prep: { path: "bin/b21_acceptance_prep.rb", status: "PASS" },
      b21_acceptance_fly_mjs: { path: "bin/b21_acceptance_fly.mjs", status: steps["status"] },
      b21_mcp_e2e_fly: { path: "bin/b21_mcp_e2e_fly.rb", status: e2e["status"], artifact: File.basename(e2e_path.to_s) },
      b21_fly_smoke: { path: "bin/b21_barista_board_fly_smoke.rb", status: smoke["status"] }
    },
