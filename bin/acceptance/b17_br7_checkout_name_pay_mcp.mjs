 ...
 *
 *   ruby bin/acceptance/b17_br7_checkout_name_pay_prep_fly.rb
...
function fetchOtp(email) {
  try {
    const out = execSync(`ruby bin/fly-tools/fetch_fly_otp.rb ${JSON.stringify(email)}`, {
      encoding: "utf8",
      cwd: root,
      timeout: 120000
    })
...
async function run() {
  ...
  const artifact = {
    ...
    playwright: {
      script: "bin/acceptance/b17_br7_checkout_name_pay_mcp.mjs",
      prep: "bin/acceptance/b17_br7_checkout_name_pay_prep_fly.rb",
      steps,
