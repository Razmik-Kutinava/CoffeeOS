...
class BlockFStockFlowTest < ActionDispatch::IntegrationTest
  ...
  # Soft-fail TASK_93-A R5: заказ accepted, склад не трогаем, audit о shortfall.
  test "sale accepted when stock insufficient (soft-fail + audit)" do
    ...
    post "/shop/api/orders",
      headers: headers,
      params: shop_order_params(email: email, name: "Negative soft", payment_method: "card"),
      as: :json
    assert_response :success

    stock = IngredientTenantStock.find_by!(tenant_id: tenant.id, ingredient_id: ingredient.id)
    assert_equal 10.to_d, stock.qty, "stock unchanged on soft-fail"
