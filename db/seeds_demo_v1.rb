...
[demo] Готово:
  ...
  Организация Alt: #{result.organization_alt.name} (#{result.organization_alt.slug})
  ...
  Точка C: #{result.tenant_c.name} (#{result.tenant_c.slug}) id=#{result.tenant_c.id}
  ...
  PTS: A=#{result.pts_a_count}, B=#{result.pts_b_count}, C=#{result.pts_c_count}
  ...
  Shop C: #{demo_shop_public_url(result.tenant_c)}
